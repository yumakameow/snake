...
function gameEngine() {
    ...
    if (isCollide(snakeArr)) {
        ...
        document.getElementById('score').innerHTML = `ОЧКО:0`;
        if (score > highScore) {
            highScore = score;
            document.getElementById('highScore').innerHTML = `РЕЕОРД:${highScore}`;
        }
        snakeArr = [{ x: 9, y: 9 }];
        score = 0;
    }
    ...
    if (snakeArr[0].y === food.y && snakeArr[0].x === food.x) {
        ...
        document.getElementById('score').innerHTML = `ОЧКО:${score}`;
